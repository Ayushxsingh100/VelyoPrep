...
export const VaultScreen: React.FC<VaultScreenProps> = ({
  ...
  const [customLinks, setCustomLinks] = useState<{ id: string; label: string; url: string }[]>(() => {
    try {
      ...
      return saved ? JSON.parse(saved) : [
        { id: "1", label: "My Main Project Demo", url: "https://my-best-project.vercel.app" },
        { id: "2", label: "Shared Drive Portfolio", url: "https://drive.google.com" }
      ];
    } catch (e) {
      return [];
    }
  ...
  // Compile active professional & coding links for display in copyable vault
  ...
  // Premium Apple-inspired variables for consistent, elegant appearance
  ...
  const handleOpenEdit = () => {
    ...
    setEditLeetcode(linkLeetcode);
    setEditCodeforces(linkCodeforces);
    setEditCodechef(linkCodechef);
    setEditHackerRank(linkHackerRank);
        ...
        {isEditing ? (
          ...
          >
            ...
            <div className="space-y-3.5 max-h-[460px] overflow-y-auto pr-1 pb-10 scrollbar-none">
              ...
              {/* Extra Document Simulator inside edit */}
              <div className="space-y-1">
                ...
                <div className={`${cardClass} py-2.5 px-3.5`}>
                  <div className="flex items-center justify-between py-1">
                    ...
                    <button
                      onClick={() => {
                        const name = prompt("Enter simulated resume filename:", "SDE Core Backend Resume.pdf");
                        if (name) {
                          setResumes([
                            {
                              id: `res_${Date.now()}`,
                              name,
                              version: "v2.0",
                              storagePath: `supabase/storage/vault/${name}`,
                              isDefault: true,
                          ...
                          showToast("Simulated resume uploaded", "success");
                        }
                      }}
                      className={`px-2.5 py-1.5 ${isDark ? "bg-blue-500/10 hover:bg-blue-500/15 text-[#0A84FF]" : "bg-blue-50 hover:bg-blue-100 text-[#007AFF]"} text-[10px] font-bold rounded-full transition-colors cursor-pointer`}
                    >
                      Change
                    </button>
                  </div>
              ...
              {/* Troubleshooting/Sync Sandbox Panel */}
              <div className="space-y-1">
          ...
          >
            {/* Header section with tiny typography */}
            ...
            {/* 1. Profile section card (Elegant iOS-style Pass) */}
            <div className={`${cardClass} py-3 px-3.5 space-y-3`}>
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center space-x-3 min-w-0">
                  {/* Profile monogram with premium ring */}
                  <div className={`w-10 h-10 rounded-full overflow-hidden flex items-center justify-center shrink-0 border ${isDark ? "bg-blue-500/10 border-blue-500/20 text-blue-400" : "bg-blue-50 border-blue-100 text-blue-600"}`}>
              ...
              {/* Education / Career quick badges inside the ID card container */}
            ...
            {/* 6. One Primary Button "Edit Profile" */}
            <div className="pt-1">
              <button
