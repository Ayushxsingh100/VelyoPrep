...
const getCompanyLogo = (companyName: string, isDark: boolean) => {
  ...
  // If we can get a beautiful, clean circular brand badge with custom coloring
...
export const DashboardScreen: React.FC<DashboardScreenProps> = ({
  ...
  // --- 1. SYSTEM OF RECORD / OFFLINE FALLBACK ---
  ...
  // --- 2. TODAY'S PRIORITY (EXACTLY ONE COMPACT CARD) ---
  const todayPriority = useMemo(() => {
    // 1st Priority: Immediate Interview scheduled
    ...
    // 2nd Priority: Active Assessment
    ...
    // 3rd Priority: Offers pending acceptance
    ...
    // 4th Priority: First uncompleted milestone
  ...
  // --- 3. TODAY'S CHECKLIST (MAX 3 ACTIONABLE ITEMS) ---
  ...
  // --- 4. CONTINUE WHERE YOU LEFT OFF (EXACTLY ONE ELEMENT) ---
  const lastInteractedItem = useMemo(() => {
    // Priority 1: Edit last resume version
    ...
    // Priority 2: Prepare last application
  ...
  const hasOA = applications.some(a => ["OA Scheduled", "OA Completed"].includes(a.status));
  ...
  return (
    ...
    >
      ...
      {/* Dynamic low-opacity ambient light behind active content (1.5% opacity) */}
      ...
      {systemStatus.error ? (
        /* PERSISTENT SANDBOX RESILIENT STATE */
        <motion.div 
        ...
        <>
          {/* ② TODAY'S PRIORITY (COMPACT HERO CARD - HIGHEST VISUAL IMPORTANCE) */}
          <motion.div variants={itemVariants} className="space-y-2 relative">
            ...
            <div className="relative group">
              {/* Soft ambient gradient glow (almost invisible, 2% opacity) behind card */}
              ...
              >
                {/* 1px subtle top accent edge to enhance depth */}
                ...
                {/* Soft interior depth radial glow */}
                ...
                <div className="flex items-center gap-2.5 shrink-0 ml-4">
                  ...
                  {/* Elegant chevron perfectly centered */}
          ...
          {/* ③ PLACEMENT JOURNEY PROGRESS (INTERACTIVE FUNNEL BENTO GRID) */}
          <motion.div variants={itemVariants} className="space-y-2">
            ...
            {/* Expanded Tray with list of applications in the selected stage */}
            <AnimatePresence>
              {selectedFunnelStage && (
                <motion.div
          ...
          {/* ④ TODAY'S CHECKLIST (MAX 3 ITEMS, COMFORTABLE TOUCH TARGETS) */}
          <motion.div variants={itemVariants} className="space-y-2">
                ...
                todayChecklist.map((task) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Clean relative date indicator on the right */}
          ...
          {/* ⑤ CONTINUE WHERE YOU LEFT OFF (HIGH-DENSITY FLUID CELL) */}
          <motion.div variants={itemVariants} className="space-y-2">
            <h2 className="text-[10px] font-medium text-zinc-500 uppercase tracking-widest select-none pl-1">
          ...
          {/* ⑥ ONE COMPACT PRIMARY CTA (SATISFYING TAPPING HEIGHT) */}
